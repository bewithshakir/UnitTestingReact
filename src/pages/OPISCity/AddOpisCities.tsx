...
export interface AddOpisProps {
  version: string;
}

const formStatusProps: FormStatusProps = formStatusObj;

const AddOpis: React.FC<AddOpisProps> = () => {
  ...
  useEffect(() => {
    if (stateList?.data?.states?.length) {
      setStates(
        stateList.data.states.map((obj: any) => ({
          label: obj.state.trim(),
          value: obj.state.trim(),
    ...
    if (citiesList?.data?.cities?.length) {
      setCities(
        citiesList.data.cities.map((obj: any) => ({
          label: obj.city.trim(),
          value: obj.cityId,
        }))
      );
