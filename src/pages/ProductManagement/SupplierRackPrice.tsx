...
export default function SupplierRack({ isDisabled, formik, setSupplierPrice, resetSupplierValue }: props) {
    ...
    return (
        <React.Fragment>
            ...
            <Button variant="outlined" className='supplier-modal-btn' disabled={!(formik.values.city && formik.values.supplier?.length && formik.values.branded?.length && formik.values.actualProduct?.length) || isDisabled} >
                <div className='price-span' onClick={handleClickOpen}> {formik.values.opisName ? `Supplier price $${formik.values.supplierPrice}` : "Choose the supplier price"} </div> {formik.values.opisName && <div className='clear-price-cross' onClick={clearPriceOnBtn}><IconButton edge='start'  >
