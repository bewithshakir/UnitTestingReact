...
import {
  AddParkingLotForm,
  addLotFormInitialValues,
  lotContact,
  orderSchDel,
} from '../../../../models/ParkingLotModel';
import AddParkingLotValidationSchema from '../validation';
import {
  useCreateLot,
  useEditParkingLot,
  useGetContactTypes,
  useGetParkingLotData,
  useGetTimeZones,
} from '../queries';
...
function AddLotForm(): React.ReactElement {
  ...
  const isFormValidated = useShowConfirmationDialogBoxStore(
    (state) => state.setFormFieldValue
  );
  const [isTrigger, setIsTrigger] = useState(false);
  const setParkingLotIdCreated = useAddedParkingLotIdStore(
    (state) => state.setParkingLotId
  );
  ...
  const onAddLotError = (err: any) => {
    ...
    try {
      ...
      setAPIResponse(true);
      setFormStatus({
        message: data?.error?.message || formStatusProps.error.message,
        type: 'Error',
      });
  ...
  useEffect(() => {
    if (timeZoneList?.data?.timezones?.length) {
      setTimeZones(
        timeZoneList.data.timezones.map((obj: any) => ({
          label: obj.timezoneNm,
          value: obj.timezoneCd,
        }))
      );
    }
  }, [timeZoneList]);
  ...
  const onAddLotSuccess = (data: any) => {
    ...
    timerRef.current = setTimeout(() => {
      setAPIResponse(false);
      navigate(
        `/customer/${addedCustomerId}/parkingLots/viewLot/${data?.data?.deliveryLocationId.toString()}`
      );
    }, 6000);
  };
  ...
  const { mutate: editParkingLot } = useEditParkingLot(
    addedLotId as string,
    onEditLotSuccess,
    onEditLotError
  );
  ...
  const populateDataInAllFields = (dataToPopulate: any) => {
    formik.setFieldValue(
      'lotName',
      dataToPopulate?.data?.lot?.deliveryLocationNm
    );
    formik.setFieldValue('lotId', dataToPopulate?.data?.lot?.lotId);
    formik.setFieldValue(
      'addressLine1',
      dataToPopulate?.data?.lot?.addressLine1
    );
    formik.setFieldValue(
      'addressLine2',
      dataToPopulate?.data?.lot?.addressLine2
    );
    ...
    formik.setFieldValue(
      'jurisdictionId',
      dataToPopulate?.data?.lot?.taxJurisdictionId
    );
    dataToPopulate?.data?.lot?.deliveryLocationContact.map(
      (obj: any, index: number) => {
        formik.setFieldValue(
          `locationContact[${index}].lotContactId`,
          obj.locationContactId
        );
        formik.setFieldValue(
          `locationContact[${index}].firstName`,
          obj.contactFirstNm
        );
        formik.setFieldValue(
          `locationContact[${index}].lastName`,
          obj.contactLastNm
        );
        formik.setFieldValue(
          `locationContact[${index}].email`,
          obj.contactEmailId
        );
        formik.setFieldValue(
          `locationContact[${index}].phoneNumber`,
          obj.contactPhoneNo
        );
      }
    );
  ...
  useEffect(() => {
    if (contactTypeList?.data.length) {
      const primaryContactObj = contactTypeList.data.find(
        (contactType: any) =>
          contactType.locationContactNm.toLowerCase() === 'primary'
      );
      const secContactObj = contactTypeList.data.find(
        (contactType: any) =>
          contactType.locationContactNm.toLowerCase() === 'secondary'
      );
  ...
  const createAddLotPayload = (form: AddParkingLotForm) => {
    const apiPayload = {
      ...
      location_contact: form.locationContact.map((contactObj: any, index) => ({
        location_contact_type_cd:
          index === 0 ? primaryContactType : secondaryContactType,
              ...
              <Grid container mt={1}>
                ...
                <Grid item xs={12} md={6} pr={2.5} pb={2.5}>
                  <Input
                    ...
                    type='text'
                    helperText={
                      formik.touched.lotName && formik.errors.lotName
                ...
                <Grid item xs={12} md={6} pl={2.5} pb={2.5}>
                  <Input
                    ...
                    type='text'
                    helperText={
                      formik.touched.lotId && formik.errors.lotId
                        ? formik.errors.lotId
                        : undefined
                    }
                    error={
                      formik.touched.lotId && formik.errors.lotId ? true : false
                    }
                    description=''
                    disabled={isDisabled}
                    required
                ...
                <Grid item xs={6} pr={2.5}>
                  <Input
                    ...
                    type='text'
                    helperText={
                      formik.touched.jurisdictionId &&
                      formik.errors.jurisdictionId
                        ? formik.errors.jurisdictionId
                        : undefined
                    }
                    error={
                ...
                <Grid item xs={12} md={6} pr={2.5} pb={2.5}>
                  <Select
                    ...
                    helperText={
                      formik.touched.productDelFreq &&
                      formik.errors.productDelFreq
                        ? formik.errors.productDelFreq.value
                        : undefined
                    }
                    error={
                      formik.touched.timeZone && formik.errors.timeZone
                    ...
                    isDisabled={isDisabled}
                    onBlur={() => {
                      formik.setFieldTouched('productDelFreq');
                      formik.validateField('productDelFreq');
                    }}
                  />
                </Grid>
                <Grid item md={12} mt={5} mb={3}>
                ...
                <FieldArray
                  ...
                  render={() => (
                    <React.Fragment>
                      {formik.values.orderScheduleDel.map(
                        (orderSchObj, index) => (
                          <Grid container key={index}>
                            <Grid item md={3} pr={2.5} pb={2.5}>
                              <DatePickerInput
                                ...
                                value={
                                  formik.values.orderScheduleDel[index].fromDate
                                }
                                onChange={formik.setFieldValue}
                                disabled
                                onClose={() => {
                                  formik.setFieldTouched(
                                    `orderScheduleDel[${index}].fromDate`
                                  );
                            ...
                            <Grid item md={3} pl={2.5} pr={2.5} pb={2.5}>
                              <DatePickerInput
                                ...
                                value={
                                  formik.values.orderScheduleDel[index].toDate
                                }
                                onChange={formik.setFieldValue}
                                disabled
                                onClose={() => {
                                  formik.setFieldTouched(
                                    `orderScheduleDel[${index}].toDate`
                                  );
                                  formik.validateField(
                                    `orderScheduleDel[${index}].toDate`
                                  );
                            ...
                            >
                              <MultiSelect
                                ...
                                value={
                                  formik.values.orderScheduleDel[index]
                                    .productDelDays
                                }
                                onChange={formik.setFieldValue}
                                disabled
                                onBlur={() => {
                                  formik.setFieldTouched(
                                    `orderScheduleDel[${index}].productDelDays`
                                  );
                                  formik.validateField(
                                    `orderScheduleDel[${index}].productDelDays`
                                  );
                            ...
                            <Grid item md={3} pr={2.5} pb={2.5}>
                              <TimePicker
                                ...
                                disabled
                                helperText={
                                  formik?.errors?.orderScheduleDel &&
                                  formik?.touched?.orderScheduleDel &&
                                  formik.touched?.orderScheduleDel?.[index]
                                    ?.startTime &&
                                  (
                                    formik.errors?.orderScheduleDel?.[
                                      index
                            ...
                            <Grid item md={3} pl={2.5} pr={2.5} pb={2.5}>
                              <TimePicker
                                ...
                                value={
                                  formik.values.orderScheduleDel[index].endTime
                                }
                                onChange={formik.setFieldValue}
                                disabled
                                helperText={
                      ...
                      <Grid item md={12} mt={2} mb={4}>
                        ...
                        >
                          <span className='add-icon-span'>
                            <PlusIcon color={theme['--Secondary-Background']} />
                          </span>
                          <Typography
                            variant='h3'
                            component='h3'
                ...
                <FieldArray
                  ...
                  render={(arrayHelpers) => (
                    <React.Fragment>
                      {formik.values.locationContact.map(
                        (contactList, index) => (
                          <Grid container key={index}>
                            <Grid item md={12} mt={2} mb={1}>
                              ...
                              >
                                {index < 1
                                  ? 'Primary Lot Contact'
                                  : 'Secondary Lot Contact (Optional)'}
                              </Typography>
                            </Grid>
                            <Grid item xs={12} md={6} pr={2.5} pb={2.5}>
                              <Input
                                ...
                                disabled={isDisabled}
                                helperText={
                                  formik?.errors?.locationContact &&
                                  formik?.touched?.locationContact &&
                                  formik.touched?.locationContact?.[index]
                                    ?.firstName &&
                                  (
                                    formik.errors?.locationContact?.[
                                      index
                  ...
                  {isSavCancelShown && (
                    <Box className='form-action-section'>
                      ...
                      <Button
                        ...
                        disabled={
                          !formik.isValid || !formik.dirty || isDisabled
                        }
                      >
                        {t('buttons.save')}
                      </Button>
